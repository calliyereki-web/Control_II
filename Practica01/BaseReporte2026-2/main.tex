...
Si a un sistema $G(s)$ cualquiera le aplicamos en la entrada una señal en forma de seno o coseno, la manera en que cambian la amplitud y la fase de la señal de salida depende de cómo son y en dónde están ubicados los polos y ceros de la función de transferencia de nuestra planta (\cite{notasclase}). 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{img/entrada_salida.png}
...
\begin{figure}[H]
\centering
    \includegraphics[width=0.4\linewidth]{img/Circuito_RC.png}
...
Para la magnitud $|G(jw)|$ recordemos que toda función de transferencia se define como el cociente de la salida entre la entrada:
...
\begin{equation}
    |G(jw)| = \left|\frac{a(a-jw)}{a^2+w^2}\right| = \frac{|a(a-jw)|}{a^2+w^2}
\end{equation}
...
$[\frac{\text{rad}}{\text{s}}]$ & \text{[Hz]} & \text{[V]} & \text{[V]} & \text{[dB]} & \text{[ms]} & $[^\circ]$ \\
\midrule
